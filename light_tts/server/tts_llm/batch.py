...
from light_tts.server.core.objs import ShmReqManager, Req
...
class Batch:
    ...
    def filter_out_finished_req(self, shm_req_manager: ShmReqManager, append_prefill_req_ids: List[int]):
        unfinished_req_ids = []
        for req in self.reqs:
            ...
            if req.shm_infer_released:
                logger.info(f"router release req_id {req.request_id}")
                shm_req_manager.put_back_req_obj(req)
                req = None
            elif req.request_id not in append_prefill_req_ids:
